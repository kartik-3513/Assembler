...
function assemble() {
	...
	for (var i = 0; i < assemblyArr.length; i++) {
		...
		if (instruction[0].includes('OG')) {
			...
			instruction[0].includes('LOAD') ||
			instruction[0].includes('STORE')
		) {
			regmem(instruction);
			lc += 1;
			continue;
		} else if (
			instruction[0].includes('JMPZ') ||
			instruction[0].includes('JMPN') ||
			instruction[0].includes('JMPU') ||
			instruction[0].includes('CALL')
		) {
			nullmem(instruction);
			lc += 1;
			continue;
		} else if (
			instruction[0].includes('ADD') ||
			instruction[0].includes('INC') ||
			instruction[0].includes('MOV') ||
			instruction[0].includes('AND') ||
			instruction[0].includes('OR') ||
			instruction[0].includes('NOT') ||
			instruction[0].includes('XOR')
		) {
			regreg(instruction);
			lc += 1;
			continue;
		} else if (
			instruction[0].includes('CILR') ||
			instruction[0].includes('CILL') ||
			instruction[0].includes('SHR') ||
			instruction[0].includes('SHL')
		) {
			nullreg(instruction);
			lc += 1;
			continue;
		} else if (
			instruction[0].includes('RET') ||
			instruction[0].includes('CINT') ||
			instruction[0].includes('CMIF') ||
			instruction[0].includes('INPT') ||
			instruction[0].includes('OUTP') ||
			instruction[0].includes('CINF') ||
			instruction[0].includes('COUF')
		) {
	...
	for (line of assemblyArr) {
		...
		if (keys_regmem.includes(operation)) {
			...
			if (instruction[2] == undefined || instruction[2] == '') {
				error += `${i + 1}: Memory reference expected.`;
				break;
			}
